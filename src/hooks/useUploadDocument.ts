  ...
  const uploadDocument = async (options: DocumentUploadOptions) => {
    ...
    try {
      ...
      if (originalDocumentId && currentVersion) {
        ...
        documentData.original_document_id = originalDocumentId;
        documentData.version = currentVersion + 1;
      ...
      if (salesStage && entityType === 'sales_process') {
        documentData.step = salesStage;
      }
      
      // Insert document record
      const { data: documentData, error: documentError } = await fromDocumentTable(tableName)
        .insert(documentData)
        .select()
        .single();
