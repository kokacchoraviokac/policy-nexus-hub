...
export const parsePolicyCSV = (csvData: string): Partial<Policy>[] => {
  ...
  return result.data.map((row: any) => {
    ...
    return {
      ...
      insured_name: row.insured_name || row.InsuredName || row.policyholder_name || row.PolicyholderName,
      ...
      product_name: row.product_name || row.product || row.Product || row.ProductName,
      product_code: row.product_code || row.ProductCode,
      notes: row.notes || row.Notes,
      payment_frequency: row.payment_frequency || row.PaymentFrequency || 'annual',
      commission_type: row.commission_type || row.CommissionType || 'automatic',
